...
def get_book_link(book_name):
    ...
    html = requests.get(
        "http://www.quanshuwang.com/modules/article/search.php?searchkey=%s&searchtype=articlename&searchbuttom.x=0&searchbuttom.y=0" % repr(book_name)[1:-1])
    return html
...
class ParseContent:
    ...
    def parsePage(self, num):
        ...
        for name in bsObj.findAll("div", {"id": "content"}):
            ...
            Name = Name.replace(
                '<div class="mainContenr" id="content"><script type="text/javascript">style5();</script>', '')
            Name = Name.replace(
                '<script type="text/javascript">style6();</script></div>', '')
            fi.write(Name)
        fi.write('\n')
    ...
    def done(self, Filetype, st, ed):
        ...
        en.sendEmailWithAttr(config.receiver, config.username,
                             config.password, "%s.mobi" % self.book_name)
